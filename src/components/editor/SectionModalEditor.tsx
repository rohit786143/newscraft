...
export type ActiveTarget = 'heading' | 'kicker' | 'subheading' | 'image' | 'body' | 'bullets' | 'border';
...
export const SectionModalEditor: React.FC<SectionModalEditorProps> = ({
  ...
  const handleImage2FileChange = (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const objectUrl = URL.createObjectURL(file);
  ...
  const handleImage3FileChange = (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const objectUrl = URL.createObjectURL(file);
  ...
  const parseDecimalSize = (val?: string | number, defaultVal: number = 14) => {
                                  ...
                                  col2Photo = (
                                    ...
                                    >
                                      ...
                                      {localSection.caption2 && (
                                        <div
                                          className="text-[9.5px] italic text-slate-700 pt-0.5"
                                          style={{ textAlign: ((localSection.caption2Align || localSection.captionAlign) as any) || 'left' }}
                                        >
                                          {localSection.caption2}
                                        </div>
                                      )}
                                  ...
                                  col3Photo = (
                                    ...
                                    >
                                      ...
                                      {localSection.caption3 && (
                                        <div
                                          className="text-[9.5px] italic text-slate-700 pt-0.5"
                                          style={{ textAlign: ((localSection.caption3Align || localSection.captionAlign) as any) || 'left' }}
                                        >
                                          {localSection.caption3}
                                        </div>
                                      )}
                      ...
                      <div className="flex items-center bg-slate-950 border border-slate-800 rounded-lg p-0.5">
                        {['left', 'center', 'right'].map((a) => (
                          <button
                            key={a}
                            type="button"
                            onClick={() => updateField('subtitleAlign', a as any)}
                            className={`px-2 py-1 rounded text-[10.5px] font-bold ${

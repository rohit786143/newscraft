...
/* ─────────────────────────────────────────────
   ROOT PAGE  /
   • Unauthenticated  → show Login UI directly
...
function PressCraftStudioContent() {
    ...
    fetch('/api/auth/me')
      ...
      .then((data) => {
        if (data.authenticated && data.user) {
          ...
          // Not logged in – show login form (authChecked stays false → login UI)
          setAuthChecked(true);
        }
      })
      .catch(() => {
        setAuthChecked(true); // show login form on error too
      });
  }, [router]);
  ...
  // ─────── Loading state (before first auth check) ───────
  if (!authChecked) {
    return (
      <div className="w-screen h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-300 font-sans">
        ...
        <div className="flex items-center gap-2 text-sm font-semibold text-slate-200">
          ...
          <span>PressCraft Studio लोड हो रहा है...</span>
        </div>
      </div>
    );
  }

  // ─────── EDITOR VIEW (authenticated non-admin) ───────
  if (currentUser) {
        ...
        {isImpersonating && (
          <div className="bg-gradient-to-r from-amber-600 via-amber-700 to-amber-600 text-white px-4 py-2 text-xs font-bold flex items-center justify-between shadow-xl z-50 shrink-0 border-b border-amber-400">
            <div className="flex items-center gap-2">
              ...
              <span>
                Viewing as <strong className="underline text-amber-100">{currentUser.name}</strong> ({currentUser.newspaperName || currentUser.username})
                <span className="bg-amber-800/80 px-2 py-0.5 rounded text-[10px] ml-1 uppercase tracking-wider font-extrabold border border-amber-500">Admin Mode</span>
              </span>
            </div>
            <button
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center items-center p-4 relative font-sans antialiased">
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-8 shadow-xl z-10">
        ...
        {/* DEVICE NOT REGISTERED MODAL */}
        {showDeviceModal && (
          <div className="p-4 rounded-2xl mb-6 text-xs leading-relaxed bg-amber-50 border border-amber-300 text-amber-900 shadow-sm">
            ...
            <div className="mt-3 p-3 bg-white border border-amber-300 rounded-xl flex items-center justify-between gap-2 shadow-inner">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer ${copied ? 'bg-emerald-600 text-white' : 'bg-amber-500 hover:bg-amber-600 text-white shadow-sm'}`}
              >
            ...
            <div className="mt-3 flex items-center justify-between pt-1 border-t border-amber-200/60">
              ...
              <a
                href={`https://api.whatsapp.com/send?phone=919857640014&text=${encodeURIComponent(`नमस्ते एडमिन, कृपया मेरे नए डिवाइस को PressCraft Studio में एक्टिवेट करें।\nUsername: ${username || 'मेरा यूज़रनेम'}\nDevice ID: ${deviceId}`)}`}
                target="_blank"
                rel="noreferrer"
        ...
        {/* ERROR / EXPIRED / BLOCKED */}
        {!showDeviceModal && errorMsg && (
          <div className={`p-4 rounded-2xl mb-6 text-xs leading-relaxed border ${isExpired || isBlocked ? 'bg-rose-50 border-rose-200 text-rose-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
              ...
              <div>
                ...
                {(isExpired || isBlocked) && (
                  <p className="mt-2 text-[11px] text-amber-800 font-bold">
                    ...
                    <a href="https://api.whatsapp.com/send?phone=919857640014" target="_blank" rel="noreferrer" className="underline font-black text-emerald-800">
                      +91 98576-40014 (WhatsApp)
                    </a>{' '}
                    to unblock or renew.
                  </p>
                )}
